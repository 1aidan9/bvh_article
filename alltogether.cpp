...
namespace Tmpl8 {
	...
	void TLAS::Build()
	{
		...
		while (nodeIndices > 1)
		{
			...
			if (A == C)
			{
				...
				newNode.leftRight = nodeIdxA + (nodeIdxB << 16); // Encode indices
	...
	void AllTogetherApp::Tick(float deltaTime)
	{
		...
		for (int i = 0; i < 256; i++)
		{
			mat4 R = mat4::RotateX(orientation[i].x) * mat4::RotateY(orientation[i].y) *
...
#pragma omp parallel for schedule(dynamic)
		...
		printf("tlas build: %.2fms, tracing time: %.2fms (%5.2fK rays/s)  \r", tlasTime, elapsed, sqr(630) / elapsed);
	}
