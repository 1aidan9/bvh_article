...
namespace Tmpl8
{
	...
	public:
		...
		aabb GetBounds() { return bounds; } // Needed for TLAS to see world bounds
	private:
	...
	struct TLASNode
	{
		union { struct { float3 aabbMin; uint leftRight; }; __m128 aabbMin4; };
		union { struct { float3 aabbMax; uint BLAS; }; __m128 aabbMax4; };
	...
	private:
		void UpdateNodeBounds(uint nodeIdx);
		void Subdivide(uint nodeIdx);
		float FindBestSplitPlane(TLASNode& node, int& axis, float& splitPos);
		TLASNode* tlasNode = 0;
		BVH* blas = 0;
		uint* tlasIdx = 0;
